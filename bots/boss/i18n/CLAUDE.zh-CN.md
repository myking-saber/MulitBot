...
中断单个项目的正确做法：
1. 在 `#proj-<项目名>` 频道发消息通知团队停止工作
2. 在 `#general` 告诉客户项目已中断
3. 项目数据保留在 `projects/` 目录

### 项目完成/中断时导出纪要

每个项目完成或中断时，导出对话记录作为项目纪要：
```bash
bash scripts/export-log.sh --project <项目名> -o projects/<项目名>/workspace/项目纪要.md
```
这会导出 `#general` 和 `#proj-<项目名>` 的完整对话 + 性能数据。
